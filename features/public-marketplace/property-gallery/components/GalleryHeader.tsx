...
export function GalleryHeader({ onSave, onShare, onSlideshow, saved, shareTriggerRef, slideshow }: GalleryHeaderProps) {
  const actions = [
    ...
    { active: false, icon: CloseIcon, label: "Close", onClick: () => { window.location.href = "/stays/luxury-studio-in-madinaty"; } },
  ];
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-[#E5E7EB] bg-white/95 backdrop-blur-xl lg:fixed lg:inset-x-0">
      <div className="mx-auto flex h-[58px] max-w-[1760px] items-center justify-between px-4 sm:h-[70px] sm:px-7 lg:px-8">
        <div className="flex min-w-0 items-center gap-5 sm:gap-12">
          ...
          <p className="hidden truncate text-[13px] font-semibold text-[#0F172A] sm:block">
            Luxury Studio in Madinaty <span className="mx-3 text-[#9CA3AF]">/</span> Photos
          </p>
        </div>
      ...
      <div className="px-4 pb-3 md:hidden">
        <p className="text-[11px] font-semibold text-[#64748B]">Luxury Studio in Madinaty / Photos</p>
