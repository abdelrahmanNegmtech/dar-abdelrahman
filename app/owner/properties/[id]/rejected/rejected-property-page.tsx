...
export default function RejectedPropertyPage({ id }: { id: string }) {
  ...
  async function resubmit() {
    ...
    router.push(ownerRoutes.propertyPublish(id));
  }
    ...
    <div className="owner-dashboard-frame">
      ...
      <section className="owner-dashboard-main pb-[74px]">
        ...
        <div className="owner-dashboard-content">
          <div className="owner-body hidden items-center gap-3 pb-3 text-[#5b2be0] max-[760px]:flex"><Link href="/owner/properties" className="inline-flex items-center gap-2"><Icon name="arrow-left" className="size-[14px]"/>Properties</Link><Icon name="chevron" className="size-[11px] -rotate-90"/><span className="owner-body">Property Details</span></div>
          <RejectedBanner />
        ...
        <div className="mx-8 mb-8 mt-3 flex min-h-[62px] items-center justify-end gap-[16px] rounded-lg border border-[#e7eaf2] bg-white px-5 max-[760px]:mx-3 max-[760px]:mb-5 max-[760px]:h-auto max-[760px]:flex-col max-[760px]:gap-[10px] max-[760px]:border-0 max-[760px]:px-0">
          <button onClick={()=>{if(window.confirm("Discard your unsaved changes?")) router.push(ownerRoutes.properties)}} className="owner-button-text grid h-[38px] w-[142px] place-items-center rounded-[5px] border border-[#d8deea] max-[760px]:order-2 max-[760px]:h-[44px] max-[760px]:w-full max-[760px]:border-[#9b7cff] max-[760px]:text-[#5824e6]">Discard changes</button>
...
function OwnerSidebar() { const hrefs=[ownerRoutes.dashboard,ownerRoutes.properties,ownerRoutes.bookings,ownerRoutes.calendar("1"),ownerRoutes.messages,ownerRoutes.reviews,ownerRoutes.payouts,ownerRoutes.dashboard,ownerRoutes.settings]; return <aside className="flex w-[205px] shrink-0 flex-col border-r border-[#edf0f6] px-[14px] pb-[15px] pt-[18px] max-[760px]:hidden">
  ...
  <nav className="mt-[28px] space-y-[4px]">{nav.map(([label,icon],index)=><Link href={hrefs[index]} key={label} className={`owner-button-text flex h-[42px] items-center gap-[14px] rounded-[7px] px-[11px] transition-colors ${index===1?"bg-[#f5f0ff] text-[#5b2be0]":"text-[#505a7e] hover:bg-[#faf8ff] hover:text-[#5b2be0]"}`}><OwnerNavIcon name={icon} />{label}</Link>)}</nav>
  <div className="mt-auto space-y-[12px]"><div className="flex h-[48px] items-center gap-[10px] rounded-[6px] border border-[#e3e7ef] px-[9px]"><Image src="/owner-selfie-ahmed-reference.png" alt="" width={28} height={28} className="size-[28px] rounded-full" quality={90}/><span className="owner-body flex-1"><b className="owner-label block">Ahmed Hassan</b><small className="owner-helper">Host</small></span><Icon name="chevron" className="size-[12px]"/></div><div className="owner-body flex h-[44px] items-center gap-[9px] rounded-[6px] border border-[#e3e7ef] px-[10px] text-[#596381]"><span className="owner-body grid size-[16px] place-items-center rounded-full border border-current">?</span>Help Center</div></div>
</aside>; }
