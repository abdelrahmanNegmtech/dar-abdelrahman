...
export default function RejectedPropertyPage({ id }: { id: string }) {
  ...
  return <div className="min-h-screen bg-[#fbfcff] pb-[74px] text-[#10183b]">
        ...
        <div className="mx-8 mb-8 mt-3 flex min-h-[62px] items-center justify-end gap-[16px] rounded-lg border border-[#e7eaf2] bg-white px-5 shadow-[var(--shadow-card)] transition-shadow hover:shadow-[var(--shadow-card-hover)] max-[760px]:mx-3 max-[760px]:mb-5 max-[760px]:h-auto max-[760px]:flex-col max-[760px]:gap-[10px] max-[760px]:border-0 max-[760px]:px-0">
          <button type="button" disabled={submitting} onClick={discard} className="owner-button-text grid h-[38px] w-[142px] place-items-center rounded-[5px] border border-[#d8deea] transition-shadow duration-200 hover:shadow-[var(--shadow-card-hover)] active:bg-[var(--surface-muted)] focus-visible:outline-none focus-visible:shadow-[var(--shadow-focus)] disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:shadow-none max-[760px]:order-2 max-[760px]:h-[44px] max-[760px]:w-full max-[760px]:border-[#9b7cff] max-[760px]:text-[#5824e6]">Discard changes</button>
          <button type="button" disabled={submitting} aria-busy={submitting} onClick={()=>void resubmit()} className="owner-button-text h-[38px] w-[208px] rounded-[5px] bg-[#5824e6] text-white shadow-[0_7px_18px_rgba(88,36,230,.2)] transition-shadow duration-200 hover:shadow-[var(--shadow-card-hover)] active:translate-y-px focus-visible:outline-none focus-visible:shadow-[var(--shadow-focus)] disabled:cursor-wait disabled:opacity-60 disabled:hover:shadow-none max-[760px]:h-[44px] max-[760px]:w-full">{submitting?"Resubmitting…":"Make changes & resubmit"}</button>
        </div>
  </div>;
}
