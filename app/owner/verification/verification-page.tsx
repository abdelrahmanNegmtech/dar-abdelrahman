  ...
  return <OwnerShell active="Verification" actions={actions} wide fluid>
    <div className="owner-dashboard-content">
      ...
      <Card className="mt-6 grid grid-cols-4 divide-x divide-[#e4e7ee] px-6 py-5 max-[1050px]:grid-cols-2 max-[1050px]:gap-y-6 max-[560px]:grid-cols-1 max-[560px]:divide-x-0">
        ...
        <Stat label="Progress"><b className="owner-number-md">{progress}% <span className="owner-helper">complete</span></b><div className="mt-3 h-1.5 rounded bg-[#e8e9ee]"><div className="h-full rounded bg-[var(--brand)]" style={{ width: `${progress}%` }} /></div></Stat>
      ...
      <div className="mt-5 grid grid-cols-[minmax(0,1.28fr)_minmax(0,1.05fr)_minmax(280px,.55fr)] gap-[18px] max-[1250px]:grid-cols-2 max-[760px]:grid-cols-1">
        <div className="space-y-4">
          ...
          <Card className="p-5">
            ...
            <div className="mt-4 grid grid-cols-2 gap-4 max-[560px]:grid-cols-1">
              ...
              <label
                className="grid min-h-28 cursor-pointer place-items-center rounded border border-dashed border-[#cbd2df] text-center outline-none focus-within:ring-2 focus-within:ring-[var(--brand)] focus-within:ring-offset-2"
              ...
              >
                ...
                <span className="owner-helper">
                  <Icon name="cloud-upload" className="mx-auto mb-1 size-8 text-[var(--brand)]"/>
                  <span className="block">Drag & drop files here</span>
                  <span className="block">or <span className="text-[var(--brand)]">click to upload</span></span>
                  <span className="owner-helper mt-2 block text-[#778096]">PDF, JPG, PNG up to 10MB</span>
                </span>
              </label>
            </div>
            <a download href="/authorization-template.txt" className="owner-button-text mt-4 flex h-9 w-fit min-w-[240px] items-center justify-start gap-2 rounded border border-[var(--brand)] bg-white px-4 text-[var(--brand)]"><Icon name="download" className="size-4 text-[var(--brand)]"/>Download authorization template</a>
          </Card>
        ...
        <aside className="space-y-[14px] max-[1250px]:col-span-2 max-[760px]:col-span-1">
          <OwnerTrustScore/>
          <MissingRequirements/>
          <Card className="p-5"><b className="owner-card-title">Linked properties</b>{properties.map(p=><Link href={`/owner/properties/${p.id}`} key={p.id} className="mt-4 flex gap-3"><Image src={p.image} alt="" width={44} height={44} className="size-11 rounded object-cover"/><span className="owner-helper"><b className="owner-label block">{p.name}</b>{p.location}<small className="owner-badge block">{p.status}</small></span></Link>)}<Link href="/owner/properties" className="owner-button-text mt-4 block text-center text-[var(--brand)]">View all properties</Link></Card>
  ...
  return <Card className="p-5">
    ...
    <div className="relative mx-auto mt-2 h-[92px] w-[180px]" role="img" aria-label="Owner trust score: 72 out of 100">
      <svg aria-hidden="true" viewBox="0 0 180 100" className="absolute inset-0 size-full overflow-visible">
        ...
        <path d={arc} pathLength="100" fill="none" stroke="var(--brand)" strokeWidth="12" strokeLinecap="butt" strokeDasharray="62 40"/>
...
function PayoutInput({label,value,onChange,verified=false}:{label:string;value:string;onChange:(value:string)=>void;verified?:boolean}) {
  return <label className="owner-label min-w-0">{label}<span className="relative mt-1 block"><input value={value} onChange={event=>onChange(event.target.value)} className={`owner-input-text h-9 w-full rounded border border-[#d9dee8] px-3 outline-none focus:border-[var(--brand)] ${verified?"pr-9":""}`}/>{verified?<CheckCircle aria-label="Verified" size={18} strokeWidth={1.8} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#20a55a]"/>:null}</span></label>;
}
function VerificationTimeline() {
  ...
  return <div className="relative mt-6">
    <span aria-hidden="true" className="absolute left-[8.333%] top-[13px] h-px w-[33.334%] bg-[var(--brand)] max-[560px]:hidden"/>
    ...
    <span aria-hidden="true" className="absolute left-[13px] top-[13px] hidden h-[40%] w-px bg-[var(--brand)] max-[560px]:block"/>
    <ol className="relative z-10 grid grid-cols-6 max-[560px]:flex max-[560px]:flex-col max-[560px]:gap-5">
      {steps.map((step,index)=><li key={index} className="min-w-0 text-center max-[560px]:grid max-[560px]:grid-cols-[28px_1fr] max-[560px]:items-start max-[560px]:gap-3 max-[560px]:text-left">
        <span className="flex h-7 items-center justify-center">
          {step.state==="active"?<span className="grid size-7 place-items-center rounded-full bg-[#e9e2ff] ring-2 ring-[#c7b5ff] shadow-[0_0_0_4px_rgba(108,76,245,.08)]"><span className="grid size-[14px] place-items-center rounded-full bg-[var(--brand)]"><span className="size-1 rounded-full bg-white"/></span></span>:<span className={`grid size-6 place-items-center rounded-full ${step.state==="complete"?"bg-[var(--brand)] text-white":"bg-[#d8dce5] text-white/80"}`}><Check aria-hidden="true" size={12} strokeWidth={2.2}/></span>}
        </span>
        <span className="mt-2 block max-[560px]:mt-0">
          <span className="owner-badge block leading-[15px] text-[#17213d]">{step.label}</span>
          <span className={`owner-helper mt-1 block ${step.state==="active"?"text-[var(--brand)]":"text-[#6d7589]"}`}>{step.helper}</span>
        </span>
      </li>)}
    </ol>
  </div>;
  ...
  return <div className="mt-4 grid grid-cols-4 gap-3 max-[1250px]:grid-cols-2 max-[460px]:grid-cols-1">
    {ownerTypes.map(({title,description,icon:OwnerTypeIcon}) => {
      ...
      return <button
        ...
        className={`relative flex min-h-[132px] flex-col items-center justify-start rounded-md border px-2 pb-3 pt-4 text-center outline-none transition-colors focus-visible:ring-2 focus-visible:ring-[var(--brand)] focus-visible:ring-offset-2 ${selected?"border-[var(--brand)] bg-[#faf8ff]":"border-[#e0e4ec] bg-white"}`}
      >
        {selected?<span className="absolute right-2 top-2 grid size-[16px] place-items-center rounded-full bg-[var(--brand)] text-white"><Check aria-hidden="true" size={10} strokeWidth={2.4}/></span>:null}
        <OwnerTypeIcon aria-hidden="true" size={22} strokeWidth={1.7} className={`mb-3 shrink-0 ${selected?"text-[var(--brand)]":"text-[#11183b]"}`}/>
...
function IdentityVerification({files,errors,onFile}:{files:Record<string,string>;errors:string[];onFile:(key:string,name:string)=>void}) {
  ...
  return <>
    <div className="mt-4 grid grid-cols-3 gap-3 max-[1100px]:grid-cols-1">
      {cards.map(card=><label key={card.key} className={`relative flex min-h-[174px] cursor-pointer flex-col items-center rounded-md border px-3 pb-3 pt-3 text-center outline-none focus-within:ring-2 focus-within:ring-[var(--brand)] focus-within:ring-offset-2 ${errors.includes(card.key)?"border-red-400 bg-red-50":"border-[#dfe3eb]"}`}>
        ...
        <Image src={card.image} alt={`${card.title} preview`} width={150} height={82} unoptimized={card.image.startsWith("blob:")} className="mt-3 h-[82px] w-full max-w-[150px] rounded object-cover"/>
      ...
      <div className={`relative flex min-h-[174px] flex-col items-center rounded-md border px-3 pb-3 pt-3 text-center ${errors.includes("selfie")?"border-red-400 bg-red-50":"border-[#dfe3eb]"}`}>
        ...
        <Image src={previews.selfie || "/owner-selfie-ahmed-reference.png"} alt="Owner selfie preview" width={88} height={88} unoptimized={Boolean(previews.selfie)} className="mt-3 size-[88px] rounded-[11px] object-cover object-center"/>
        <label className="owner-button-text mt-2 grid h-[29px] w-full cursor-pointer place-items-center rounded border border-[var(--brand)] text-[var(--brand)] outline-none focus-within:ring-2 focus-within:ring-[var(--brand)] focus-within:ring-offset-1">
