...
const ownerActionHover = "transition-shadow duration-200 hover:shadow-[var(--shadow-card-hover)] disabled:hover:shadow-none";
...
const payoutSummaryCards = [
  { label:"Available to withdraw", value:"EGP 24,800", icon:WalletCards, size:26, iconClass:"text-[var(--brand)]", wrapClass:"" },
  { label:"Upcoming payouts", value:"EGP 18,450", icon:CalendarDays, size:26, iconClass:"text-[var(--brand)]", wrapClass:"" },
  ...
  { label:"Total lifetime earnings", value:"EGP 286,700", icon:ChartNoAxesColumnIncreasing, size:27, iconClass:"text-[var(--brand)]", wrapClass:"" },
] as const;
...
export default function PayoutsPage(){
  ...
  const actions=<><button onClick={()=>generateDownload("dar-owner-statement.txt","DAR Owner Statement - May 2026")} className={`owner-button-text flex h-10 items-center gap-2 rounded border border-[#ccd2dd] px-4 ${ownerActionHover}`}><Icon name="download" className="size-4"/>Download statement</button><button onClick={exportCsv} className={`owner-button-text flex h-10 items-center gap-2 rounded border border-[#ccd2dd] px-4 ${ownerActionHover}`}><Icon name="upload" className="size-4"/>Export payouts</button><button onClick={()=>setModal(true)} className={`owner-button-text h-10 rounded bg-[var(--brand)] px-5 text-white ${ownerActionHover}`}>Update payout method</button></>;
    ...
    <Card className="mt-3 p-4">
      <div className="grid grid-cols-[minmax(300px,1.4fr)_minmax(150px,.6fr)_minmax(170px,.7fr)_minmax(240px,1fr)_minmax(180px,.8fr)] items-end gap-4 max-[1400px]:grid-cols-2 max-[620px]:grid-cols-1">
        <label className="relative block">
          ...
          <input value={search} onChange={e=>{const next=e.target.value;setSearch(next);setApplied(current=>({...current,search:next}))}} placeholder="Search payout ID, booking ID, guest, statement..." className="owner-input-text h-10 w-full rounded-md border border-[#dce1e9] pl-10 pr-3 outline-none focus:border-[var(--brand)]"/>
        </label>
        ...
        <label className="relative block">
          <input type="text" value={date} onChange={e=>setDate(e.target.value)} className="owner-input-text h-10 w-full rounded-md border border-[#dce1e9] px-3 pr-10 outline-none focus:border-[var(--brand)]"/>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex flex-wrap items-center gap-2">
            {[
              ...
              ["Failed","bg-[#ef3e3e]"],
              ["On hold","bg-[#9da3ae]"],
            ].map(([chip,dot])=><button key={chip} onClick={()=>{setStatus(chip);setApplied(current=>({...current,status:chip}))}} className={`owner-button-text flex h-9 items-center gap-2 whitespace-nowrap rounded-[10px] border px-3 transition-colors ${status===chip?"border-[var(--brand)] bg-[#faf8ff] text-[var(--brand)]":"border-[#dce1e9] bg-white text-[#17213d]"}`}>{dot?<span className={`size-2 shrink-0 rounded-full ${dot}`}/>:null}{chip}</button>)}
          </div>
          <div className="flex flex-wrap items-center gap-2">
            {["All","InstaPay","Bank transfer","Vodafone Cash","Paymob","Meeza"].map(chip=>{
              ...
              return <button key={chip} onClick={()=>{setMethod(value);setApplied(current=>({...current,method:value}))}} className={`owner-button-text h-9 whitespace-nowrap rounded-[10px] border px-3 transition-colors ${selectedMethod?"border-[var(--brand)] bg-[#faf8ff] text-[var(--brand)]":"border-[#dce1e9] bg-white text-[#17213d]"}`}>{chip}</button>;
            })}
          </div>
          <button onClick={()=>setApplied({search,status,method,property,date})} className={`owner-button-text ml-auto h-9 whitespace-nowrap rounded-[8px] bg-[var(--brand)] px-5 text-white ${ownerActionHover}`}>Apply filters</button>
          <button onClick={()=>{setSearch("");setStatus("All");setMethod("All methods");setDate("");setProperty("All properties");setApplied({search:"",status:"All",method:"All methods",property:"All properties",date:""})}} className={`owner-button-text h-9 whitespace-nowrap rounded-[8px] border border-[#dce1e9] bg-white px-4 text-[var(--brand)] ${ownerActionHover}`}>Clear all</button>
        </div>
      </div>
    </Card>
        ...
        <div className="grid grid-cols-[1.2fr_1fr_1fr_1fr] gap-4 max-[1199px]:grid-cols-2 max-[700px]:grid-cols-1">
          ...
          <Card className="p-4">
            ...
            <div className="owner-helper mt-4 flex flex-wrap gap-x-4 gap-y-1">
              <span className="flex items-center gap-1.5"><i className="size-2 rounded-sm bg-[var(--brand)]"/>Gross earnings</span>
            ...
            <div className="mt-4 flex h-32 items-end justify-around border-b border-[#dfe3eb]">{[88,88,76,64,64,51].map((h,i)=><div key={i} className="flex items-end gap-0.5"><span className="w-2.5 bg-[var(--brand)]" style={{height:h}}/><span className="w-2.5 bg-[#2fa84f]" style={{height:h*.65}}/><span className="w-2.5 bg-[#e6a334]" style={{height:h*.36}}/></div>)}</div>
            <div className="owner-helper mt-2 flex justify-around">{["Jan","Feb","Mar","Apr","May","Jun"].map(x=><span key={x}>{x}</span>)}</div>
          </Card>
          <Card className="p-4">
            <div className="flex justify-between gap-3"><b className="owner-card-title">Upcoming payouts</b><button className="owner-button-text whitespace-nowrap text-[var(--brand)]">View all</button></div>
            {payouts.slice(0,3).map((p,i)=><button key={p.id} onClick={()=>setSelected(p)} className="mt-4 grid w-full grid-cols-[44px_minmax(0,1fr)_auto] items-center gap-3 text-left">
              <Image src={properties.find(x=>x.id===p.propertyId)?.image||properties[0].image} alt="" width={44} height={44} className="size-11 rounded object-cover"/>
          ...
          <Card className="p-4">
            ...
            <div className="mt-4 flex gap-3 rounded border border-red-200 bg-red-50 p-3"><TriangleAlert aria-hidden="true" size={22} strokeWidth={1.8} className="mt-0.5 shrink-0 text-[#ef5c55]"/><div><b className="owner-label">Failed payout</b><p className="owner-helper">Bank rejected destination details.</p><button onClick={()=>setModal(true)} className={`owner-button-text mt-2 rounded border border-[#8e6bf0] px-3 py-1 ${ownerActionHover}`}>Fix payout method</button></div></div>
            <div className="mt-3 flex gap-3 rounded border border-[#f1ce86] bg-[#fffaf0] p-3"><Clock3 aria-hidden="true" size={22} strokeWidth={1.8} className="mt-0.5 shrink-0 text-[#eea126]"/><div><b className="owner-label">On hold payout</b><p className="owner-helper">Waiting guest dispute resolution.</p><button onClick={()=>notify("Support case opened.")} className={`owner-button-text mt-2 rounded border border-[#8e6bf0] px-3 py-1 ${ownerActionHover}`}>View case</button></div></div>
          </Card>
        </div>
        <div className="grid grid-cols-[1.1fr_1fr_1fr] gap-4 max-[1199px]:grid-cols-2 max-[700px]:grid-cols-1">
          <Card className="p-4">
            <div className="flex items-center justify-between gap-3"><b className="owner-card-title">Statements & receipts</b><button onClick={()=>generateDownload("dar-owner-statements.txt","DAR owner statements and receipts")} className="owner-button-text whitespace-nowrap text-[var(--brand)]">Download all</button></div>
            {[
          ...
          <Card className="p-4">
            ...
            <div className="mt-3 flex gap-3 rounded border border-[#e6e8ef] bg-[#f8f8fb] p-3"><SearchIcon aria-hidden="true" size={22} strokeWidth={1.8} className="shrink-0 text-[#4d5771]"/><div><b className="owner-label">Need help with a payout?</b><p className="owner-helper mt-1">Open a payout ticket and our team will assist you.</p><Link href="/owner/help-center" className="owner-button-text mt-2 inline-block text-[var(--brand)]">Open payout ticket →</Link></div></div>
          </Card>
          <Card className="p-4">
            <div className="flex items-center justify-between gap-3"><b className="owner-card-title">Recent activity</b><button className="owner-button-text text-[var(--brand)]">View all</button></div>
            {[
            ...
            ].map(([title,meta],i)=><p key={title} className="owner-body mt-4 flex gap-3"><span className={`mt-1.5 size-2 shrink-0 rounded-full ${i===1?"bg-[var(--brand)]":"bg-[#2fa84f]"}`}/><span><b className="owner-label block">{title}</b><small className="owner-helper text-[#6f788c]">{meta}</small></span></p>)}
          </Card>
        </div>
      </div>
  ...
  return <Card className="min-w-0 max-w-full">
    <div className="flex flex-wrap items-center justify-between gap-4 px-4 py-3">
      ...
      <div className="flex flex-wrap justify-end gap-2">
        {["All","Upcoming","Paid","Processing","Failed","On hold"].map(x=><button key={x} onClick={()=>onStatusChange(x)} className={`owner-badge whitespace-nowrap rounded px-3 py-1 ${activeStatus===x?"bg-[var(--brand)] text-white":"border border-[#dce1e9]"}`}>{x}</button>)}
      </div>
    </div>
      ...
      <table className="w-full table-fixed border-separate border-spacing-0 max-[1499px]:min-w-[1180px]">
        ...
        <tbody>{rows.map(p=>{
          ...
          return <tr key={p.id} onClick={()=>onSelect(p)} className={`cursor-pointer transition-colors hover:bg-[#faf9ff] ${isSelected?"bg-[#f8f5ff]":"bg-white"}`}>
            <td className={`owner-body h-[52px] border-b border-[#e8eaf0] px-2 whitespace-nowrap ${isSelected?"border-l-2 border-l-[var(--brand)]":"border-l-2 border-l-transparent"}`}>{p.id}</td>
            <td className="owner-body border-b border-[#e8eaf0] px-2 whitespace-nowrap"><Link href="/owner/bookings/request-decision" onClick={event=>event.stopPropagation()} className="text-[var(--brand)]">{p.booking}</Link></td>
            ...
            <td className="owner-body border-b border-[#e8eaf0] px-2 whitespace-nowrap"><button onClick={event=>{event.stopPropagation();runAction(p)}} className="owner-button-text text-[var(--brand)] hover:underline">{actionLabel[p.id]??"View"}</button></td>
          </tr>;
        })}</tbody>
      </table>
    </div>
...
function HoldReasonDialog({payout,close}:{payout:Payout;close:()=>void}){
  return <div className="fixed inset-0 z-50 grid place-items-center bg-[#071126]/55 p-4" role="dialog" aria-modal="true" aria-labelledby="hold-reason-title">
    <Card className="w-full max-w-md p-5">
      <div className="flex items-center justify-between"><b id="hold-reason-title" className="owner-section-title">Payout on hold</b><button onClick={close} aria-label="Close hold reason" className={`rounded ${ownerActionHover}`}><Icon name="x" className="size-5"/></button></div>
      ...
      <button onClick={close} className={`owner-button-text mt-5 h-10 w-full rounded bg-[var(--brand)] text-white ${ownerActionHover}`}>Close</button>
    </Card>
  </div>;
}

function Select({label,value,onChange,options}:{label:string;value:string;onChange:(v:string)=>void;options:string[]}){return <label className="owner-label block">{label}<span className="relative mt-1 block"><select value={value} onChange={e=>onChange(e.target.value)} className="owner-input-text h-10 w-full appearance-none rounded-md border border-[#dce1e9] bg-white px-3 pr-9 outline-none focus:border-[var(--brand)]">{options.map(x=><option key={x}>{x}</option>)}</select><ChevronDown aria-hidden="true" size={16} strokeWidth={1.8} className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-[#59637d]"/></span></label>}
...
function PayoutMethod({payMethod,setPayMethod,onEdit}:{payMethod:string;setPayMethod:(x:string)=>void;onEdit:()=>void}){
  ...
  return <Card className="w-full p-5">
    ...
    <div className="mt-4 flex w-full">
      {["InstaPay","Bank transfer","Vodafone Cash"].map(x=><button key={x} onClick={()=>setPayMethod(x)} className={`owner-badge h-9 min-w-0 flex-1 whitespace-nowrap rounded-md px-1 text-center !text-[11px] !leading-4 ${payMethod===x?"bg-[var(--brand)] text-white":"border border-[#e1e5ed] bg-white text-[#17213d]"}`}>{x}</button>)}
    </div>
    ...
    <button onClick={onEdit} className={`owner-button-text mt-4 h-10 w-full rounded-md bg-[var(--brand)] text-white ${ownerActionHover}`}>Update payout details</button>
...
function Selected({p,close,modal,download}:{p:typeof payouts[number];close:()=>void;modal:()=>void;download:()=>void}){return <Card className="self-start p-4"><div className="flex justify-between"><b className="owner-card-title">Selected payout</b><button onClick={close} className={`rounded ${ownerActionHover}`}><Icon name="x" className="size-5"/></button></div><div className="mt-5 flex justify-between"><b className="owner-section-title">{p.id}</b><Badge status={p.status}/></div><div className="mt-4 flex gap-3"><Image src={properties.find(x=>x.id===p.propertyId)?.image||properties[0].image} alt="" width={90} height={70} className="h-[70px] w-[90px] rounded object-cover"/><span className="owner-helper"><Link href={`/owner/properties/${p.propertyId}`} className="owner-card-title block">{p.property}</Link>Madinaty - Building B6</span></div><div className="owner-body mt-5 space-y-3">{[["Booking reference",p.booking],["Guest",p.guest],["Check-in","May 20, 2026"],["Check-out","May 25, 2026"],["Nights","5"],["Gross booking",money(p.gross)],["DAR commission",`- ${money(p.commission)}`],["Net payout",money(p.net)],["Payout method",p.method],["Status",p.status]].map(([a,b])=><p key={a}>{a}<b className="float-right">{b}</b></p>)}</div><hr className="my-5 border-[#e1e5ed]"/><b className="owner-card-title">Payout timeline</b>{["Booking confirmed","Guest payment verified","Stay completed","Payout scheduled","Payout sent"].map((x,i)=><p key={x} className="owner-body mt-4 flex gap-3"><span className={`mt-1 grid size-4 place-items-center rounded-full ${i<3?"bg-green-500 text-white":"border border-[var(--brand)]"}`}>{i<3?<Icon name="check" className="size-3"/>:""}</span>{x}</p>)}<div className="mt-5 space-y-2"><button onClick={download} className={`owner-button-text h-9 w-full rounded bg-[var(--brand)] text-white ${ownerActionHover}`}>Download receipt</button><button onClick={()=>generateDownload("payout-statement.txt",`Statement ${p.id}`)} className={`owner-button-text h-9 w-full rounded border border-[#d3d8e2] ${ownerActionHover}`}>Download payout statement</button><div className="grid grid-cols-2 gap-2"><Link href="/owner/help-center" className={`owner-button-text grid h-9 place-items-center rounded border border-[#d3d8e2] ${ownerActionHover}`}>Contact support</Link><button onClick={modal} className={`owner-button-text rounded border border-[#d3d8e2] ${ownerActionHover}`}>Change payout method</button></div></div></Card>}
function MethodModal({value,setValue,close,save}:{value:string;setValue:(x:string)=>void;close:()=>void;save:()=>void}){return <div className="fixed inset-0 z-50 grid place-items-center bg-[#071126]/55 p-4"><Card className="w-full max-w-md p-5"><div className="flex justify-between"><b className="owner-section-title">Update payout method</b><button onClick={close} className={`rounded ${ownerActionHover}`}><Icon name="x" className="size-5"/></button></div><div className="mt-5 space-y-2">{["InstaPay","Bank transfer","Vodafone Cash"].map(x=><button key={x} onClick={()=>setValue(x)} className={`owner-button-text h-11 w-full rounded border ${value===x?"border-[var(--brand)] bg-[#faf8ff]":"border-[#dce1e9]"}`}>{x}</button>)}</div><label className="owner-label mt-4 block">Destination<input defaultValue="ahmed@instapay" className="owner-input-text mt-1 h-10 w-full rounded border border-[#dce1e9] px-3"/></label><div className="mt-5 flex gap-3"><button onClick={close} className={`owner-button-text h-10 flex-1 rounded border ${ownerActionHover}`}>Cancel</button><button onClick={save} className={`owner-button-text h-10 flex-1 rounded bg-[var(--brand)] text-white ${ownerActionHover}`}>Save method</button></div></Card></div>}
