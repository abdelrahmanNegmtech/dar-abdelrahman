    ...
    <Card className="mt-3 p-4">
      ...
      <div className="mt-3">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button onClick={()=>setApplied({search,status,method,property,date})} className="owner-button-text ml-auto h-9 whitespace-nowrap rounded-[8px] bg-[var(--brand)] px-5 text-white transition-colors hover:bg-[var(--brand-strong)]">Apply filters</button>
          <button onClick={()=>{setSearch("");setStatus("All");setMethod("All methods");setDate("");setProperty("All properties");setApplied({search:"",status:"All",method:"All methods",property:"All properties",date:""})}} className="owner-button-text h-9 whitespace-nowrap rounded-[8px] border border-[#dce1e9] bg-white px-4 text-[var(--brand)] transition-colors hover:bg-[#f7f3ff]">Clear all</button>
        </div>
      </div>
    </Card>
...
function HoldReasonDialog({payout,close}:{payout:Payout;close:()=>void}){
  return <div className="fixed inset-0 z-50 grid place-items-center bg-[#071126]/55 p-4" role="dialog" aria-modal="true" aria-labelledby="hold-reason-title">
    <Card className="w-full max-w-md p-5">
      ...
      <button onClick={close} className="owner-button-text mt-5 h-10 w-full rounded bg-[var(--brand)] text-white">Close</button>
    </Card>
  </div>;
}
...
function PayoutMethod({payMethod,setPayMethod,onEdit}:{payMethod:string;setPayMethod:(x:string)=>void;onEdit:()=>void}){
  ...
  return <Card className="w-full p-5">
    ...
    <button onClick={onEdit} className="owner-button-text mt-4 h-10 w-full rounded-md bg-[var(--brand)] text-white transition-colors hover:bg-[var(--brand-strong)]">Update payout details</button>
...
function Selected({p,close,modal,download}:{p:typeof payouts[number];close:()=>void;modal:()=>void;download:()=>void}){return <Card className="self-start p-4"><div className="flex justify-between"><b className="owner-card-title">Selected payout</b><button onClick={close}><Icon name="x" className="size-5"/></button></div><div className="mt-5 flex justify-between"><b className="owner-section-title">{p.id}</b><Badge status={p.status}/></div><div className="mt-4 flex gap-3"><Image src={properties.find(x=>x.id===p.propertyId)?.image||properties[0].image} alt="" width={90} height={70} className="h-[70px] w-[90px] rounded object-cover"/><span className="owner-helper"><Link href={`/owner/properties/${p.propertyId}`} className="owner-card-title block">{p.property}</Link>Madinaty - Building B6</span></div><div className="owner-body mt-5 space-y-3">{[["Booking reference",p.booking],["Guest",p.guest],["Check-in","May 20, 2026"],["Check-out","May 25, 2026"],["Nights","5"],["Gross booking",money(p.gross)],["DAR commission",`- ${money(p.commission)}`],["Net payout",money(p.net)],["Payout method",p.method],["Status",p.status]].map(([a,b])=><p key={a}>{a}<b className="float-right">{b}</b></p>)}</div><hr className="my-5 border-[#e1e5ed]"/><b className="owner-card-title">Payout timeline</b>{["Booking confirmed","Guest payment verified","Stay completed","Payout scheduled","Payout sent"].map((x,i)=><p key={x} className="owner-body mt-4 flex gap-3"><span className={`mt-1 grid size-4 place-items-center rounded-full ${i<3?"bg-green-500 text-white":"border border-[var(--brand)]"}`}>{i<3?<Icon name="check" className="size-3"/>:""}</span>{x}</p>)}<div className="mt-5 space-y-2"><button onClick={download} className="owner-button-text h-9 w-full rounded bg-[var(--brand)] text-white">Download receipt</button><button onClick={()=>generateDownload("payout-statement.txt",`Statement ${p.id}`)} className="owner-button-text h-9 w-full rounded border border-[#d3d8e2]">Download payout statement</button><div className="grid grid-cols-2 gap-2"><Link href="/owner/help-center" className="owner-button-text grid h-9 place-items-center rounded border border-[#d3d8e2]">Contact support</Link><button onClick={modal} className="owner-button-text rounded border border-[#d3d8e2]">Change payout method</button></div></div></Card>}
function MethodModal({value,setValue,close,save}:{value:string;setValue:(x:string)=>void;close:()=>void;save:()=>void}){return <div className="fixed inset-0 z-50 grid place-items-center bg-[#071126]/55 p-4"><Card className="w-full max-w-md p-5"><div className="flex justify-between"><b className="owner-section-title">Update payout method</b><button onClick={close}><Icon name="x" className="size-5"/></button></div><div className="mt-5 space-y-2">{["InstaPay","Bank transfer","Vodafone Cash"].map(x=><button key={x} onClick={()=>setValue(x)} className={`owner-button-text h-11 w-full rounded border ${value===x?"border-[var(--brand)] bg-[#faf8ff]":"border-[#dce1e9]"}`}>{x}</button>)}</div><label className="owner-label mt-4 block">Destination<input defaultValue="ahmed@instapay" className="owner-input-text mt-1 h-10 w-full rounded border border-[#dce1e9] px-3"/></label><div className="mt-5 flex gap-3"><button onClick={close} className="owner-button-text h-10 flex-1 rounded border">Cancel</button><button onClick={save} className="owner-button-text h-10 flex-1 rounded bg-[var(--brand)] text-white">Save method</button></div></Card></div>}
