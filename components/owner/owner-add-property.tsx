...
function Action({children,icon,primary,disabled,onClick}:{children:string;icon:string;primary?:boolean;disabled?:boolean;onClick:()=>void}) { return <button type="button" disabled={disabled} onClick={onClick} className={`owner-card-title flex h-9 items-center gap-2 whitespace-nowrap rounded-md px-3 transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--brand)] focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-60 sm:px-5 ${primary?"bg-[var(--brand)] !text-[#FFFFFF] hover:bg-[var(--brand-strong)] active:bg-[var(--brand-strong)] disabled:!text-[#FFFFFF]":"border border-[var(--border)] bg-[var(--surface)] hover:bg-[var(--surface-muted)]"}`}><Icon name={icon} className="size-4"/>{children}</button>; }
