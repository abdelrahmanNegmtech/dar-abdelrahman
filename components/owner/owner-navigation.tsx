...
import { OWNER_CALENDAR_FALLBACK_PROPERTY_ID, OWNER_PROFILE_FALLBACK } from "@/lib/owner-portal";
...
export function getOwnerNavigation(propertyId = OWNER_CALENDAR_FALLBACK_PROPERTY_ID): OwnerNavigationItem[] {
  return [
    ...
    { label: "Calendar", href: `/owner/properties/${propertyId}/calendar-management`, icon: CalendarDays, match: (path) => path.includes("/calendar") },
      ...
      <OwnerProfileLink owner={OWNER_PROFILE_FALLBACK} className="mt-6 rounded-[var(--radius-md)] border border-[var(--sidebar-dark-border)] p-4 transition-colors hover:border-[var(--brand)] hover:bg-[var(--sidebar-dark-elevated)] focus-visible:outline-none focus-visible:shadow-[var(--shadow-focus)] active:bg-[var(--sidebar-dark-elevated)]">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <b className="block truncate text-sm font-semibold text-[var(--sidebar-dark-foreground)]">{OWNER_PROFILE_FALLBACK.name}</b>
            <p className="mt-1 text-xs text-[var(--sidebar-dark-muted)]">{OWNER_PROFILE_FALLBACK.accountLabel}</p>
            <p className="mt-1 truncate text-xs text-[var(--sidebar-dark-muted)]">{OWNER_PROFILE_FALLBACK.location}</p>
          </div>
        </div>
      </OwnerProfileLink>
